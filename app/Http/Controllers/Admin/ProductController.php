...
class ProductController extends Controller
{
  ...
  {
    $producers = Producer::select('id', 'name')->orderBy('name', 'asc')->get();
    $product = Product::select('id', 'producer_id', 'name', 'image', 'sku_code', 'display', 'cpu', 'ram', 'storage', 'graphics', 'dimensions', 'weight', 'OS', 'pin', 'information_details', 'product_introduction')
    ->whereHas('product_details', function (Builder $query) {
      $query->where('import_quantity', '>', 0);
    })->where('id', $id)->with([
